...
describe("formatCurrency", () => {
  ...
  it("should throw an error when the argument is not a number", () => {
    expect(() => formatCurrency("1234" as any)).toThrowError(
      "Invalid argument: amount must be a number"
    );
    expect(() => formatCurrency(undefined as any)).toThrowError(
      "Invalid argument: amount must be a number"
    );
    expect(() => formatCurrency(null as any)).toThrowError(
      "Invalid argument: amount must be a number"
    );
    expect(() => formatCurrency({} as any)).toThrowError(
      "Invalid argument: amount must be a number"
    );
    expect(() => formatCurrency([] as any)).toThrowError(
      "Invalid argument: amount must be a number"
